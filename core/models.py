...
class Airport(mongoengine.Document):
  ...
  elevation = fields.IntField()  # in feet
  ...
  def geolocation(self):
    return unit.Geolocation(
        unit.Angle(self.location['coordinates'][0], unit.Angle.DEGREE,
                   unit.Angle.LONGITUDE_RANGE),
        unit.Angle(self.location['coordinates'][1], unit.Angle.DEGREE,
...
class Person(mongoengine.Document):
  ...
  airman_certificates = fields.ListField(
      fields.EmbeddedDocumentField(AirmanCertificate))
...
class ComputeContext(object):
  ...
  def get_field_value(self, field_type_cls):
    return self._log_entry.get_field_value(
        field_type_cls.id(),
        self._current_participant.airman_id
        if self._current_participant else None,
...
class BasicLogEntryFieldType(LogEntryFieldType):
  CONVERTERS = [
      ...
      (LogEntryFieldDataType.DATETIME,
       lambda x: datetime.datetime.strptime(x, '%Y%m%d %H%M%S'),
       lambda x: '{:%Y%m%d %H%M%S}'.format(x)),
      (LogEntryFieldDataType.DATE,
       lambda x: datetime.datetime.strptime(x, '%Y%m%d').date(),
       lambda x: '{:%Y%m%d}'.format(x)),
      (LogEntryFieldDataType.TIMEDELTA,
       lambda x: datetime.timedelta(seconds=float(x)),
       lambda x: str(x.total_seconds())),
      (LogEntryFieldDataType.BOOLEAN, lambda x: x.upper() == 'YES',
       lambda x: 'YES' if x else 'NO'),
  ]

  def __init__(self, display_name, data_type, is_hidden=False):
    super(BasicLogEntryFieldType, self).__init__(
        display_name, is_hidden=is_hidden)
    self._data_type = data_type

    converter = [
        x for x in BasicLogEntryFieldType.CONVERTERS if x[0] == data_type
    ]
    if not converter:
...
class ObjectLogEntryFieldType(LogEntryFieldType):
  ...
  def __init__(self, display_name, object_cls, is_hidden=False):
    super(ObjectLogEntryFieldType, self).__init__(
        display_name, is_hidden=is_hidden)
...
class LogEntryFieldTypeFactory(object):
  ...
  def register(field_type_cls):
    LogEntryFieldTypeFactory._repository[
        field_type_cls.id()] = field_type_cls()
  ...
  def get_all():
    return sorted(
        LogEntryFieldTypeFactory._repository.values(),
    ...
    for field_type in field_types:
      ...
      for participant in self.participants:
        value = self.get_field_value(
            field_type.id(), participant.airman_id, default_to_shared=False)
        s += '{0:<20} | '.format(trim(value, 20))
      s += '\n'
